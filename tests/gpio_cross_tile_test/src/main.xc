...
#define TIMESTAMP_TEST_DELAY_SLACK_CLOCKS (200)

on tile[1] : port input_port = XS1_PORT_4D;
...
int main(void) {
    ...
    par {
#if EVENTS
        on tile[1] : input_gpio_with_events(i_input_port, NUM_CLIENTS, input_port, pin_map);
#else
        on tile[1] : input_gpio(i_input_port, NUM_CLIENTS, input_port, pin_map);
#endif
        par (int i = 0; i < NUM_CLIENTS; i++) {
            on tile[i%2] : read_port(i_input_port[i], i); // place on tile[0] & tile[1]
        }
    }
    return 0;
}
